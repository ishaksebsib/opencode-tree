...
export function resolveTreeRouteBodyState(input: ResolveTreeRouteBodyStateInput): TreeRouteBodyState {
  ...
  return (
    ...
    >
      <text fg={props.busy ? props.palette.branchingText : props.palette.helpText}>
        <span style={{ fg: props.palette.helpKey }}>↑/↓</span> move • <span style={{ fg: props.palette.helpKey }}>j/k</span> move •{" "}
        <span style={{ fg: props.palette.helpKey }}>Enter</span> branch • <span style={{ fg: props.palette.helpKey }}>esc</span> back
      </text>
    </box>
  )
}
...
export function TreeRouteStatusPanel(props: TreeRouteStatusPanelProps) {
  ...
  return (
    <box backgroundColor={props.palette.panelBackground} paddingLeft={1} paddingRight={1} paddingTop={0} paddingBottom={1}>
    ...
    <Show when={props.state} keyed>
      {(state: TreeRouteBodyState) =>
        state.kind === "status" ? (
          ...
          <box flexDirection="column" flexGrow={1} minHeight={0} backgroundColor={props.palette.panelBackground}>
            <TreeView rows={state.rows} selectedIndex={props.selectedIndex} width={props.treeWidth} theme={props.theme} autoFocus onFocusChange={props.onFocusChange} />
          </box>
        )
      }
    </Show>
